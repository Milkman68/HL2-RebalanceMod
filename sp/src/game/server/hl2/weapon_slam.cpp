...
BEGIN_DATADESC( CWeapon_SLAM )
	...
	DEFINE_FIELD( m_flAttachGraceTime, FIELD_TIME ),
	DEFINE_FIELD( m_bNeedToggle, FIELD_BOOLEAN ),
...
// Output :
//-----------------------------------------------------------------------------
bool CWeapon_SLAM::TripmineAttach( void )
{
...
{
	// HACK: Makes CBaseCombatWeapon happy with selecting a weapon with no ammo. 
	m_iClip2 = m_bDetonatorArmed ? 1 : 0;
	...
	{	
		...
		{
			if ( ( CanAttachSLAM() || gpGlobals->curtime - m_flAttachGraceTime <= SLAM_GRACE_PERIOD ) && !m_bThrowSatchel )
			{
			...
			else
			{
				if (m_tSlamState == SLAM_TRIPMINE_READY)
				{
					...
					SendWeaponAnim( iAnim );
					m_flWallSwitchTime = gpGlobals->curtime + SequenceDuration() * 0.75;
					m_bNeedReload = false;
				}
			}
		}
		...
		{
			if ( m_bNeedToggle && !m_bThrowSatchel && !m_bAttachTripmine )
			{
				m_bNeedToggle = false;
				if (m_tSlamState == SLAM_SATCHEL_THROW)
				{
...
bool CWeapon_SLAM::CanAttachSLAM( void )
{
	...
	Vector	vecEnd = vecSrc + (vecAiming * 64);
	...
	if (tr.fraction < 1.0)
	{
		...
		m_flAttachGraceTime = gpGlobals->curtime;
		return true;
	}
	else
	{
...
/* 		else if (CanAttachSLAM())
		{
			...
			SetSlamState(SLAM_TRIPMINE_READY); // This animation doesn't exist
		} */
		else
		{
	...
	{	
		if (CanAttachSLAM() && !slam_use_reload_toggle.GetBool() )
		{
...
{
	return m_bDetonatorArmed;
}

void CWeapon_SLAM::ItemHolsterFrame( void )
{
	// We need this check because satchels can still be detonated while holstered if damaged.
	m_bDetonatorArmed = AnyUndetonatedCharges();
	m_iClip2 = m_bDetonatorArmed ? 1 : 0;

...
// Antlion summoning
#define ANTLIONGUARD_SUMMON_COUNT		5
...
#if HL2_EPISODIC
#define ANTLIONGUARD_POISON_TO			1 // we only poison Gordon down to twelve to give him a chance to regen up to 20 by the next charge
#endif
...
{
	if ( !m_bBarkEnabled && ( hl2_episodic.GetBool() ))
...
void CNPC_AntlionGuard::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
	...
	{
		info.ScaleDamage( sk_antlionguard_dmg_scale_buckshot.GetFloat() );
	}
	else if ( info.GetDamageType() & DMG_BULLET )
	{
		info.ScaleDamage( sk_antlionguard_dmg_scale_bullet.GetFloat() );
	}
...
void CNPC_AntlionGuard::SummonAntlions( void )
{
	...
	for ( int i = 0; (m_iNumLiveAntlions < ANTLIONGUARD_SUMMON_COUNT) && (iSpawnPoint < ARRAYSIZE(sAntlionSpawnPositions)); i++ )
	{
		...
		if ( ( pdata->game.material != CHAR_TEX_DIRT ) && ( pdata->game.material != CHAR_TEX_SAND ) && ( hl2_episodic.GetBool() ))
		{
			if ( g_debug_antlionguard.GetInt() == 2 )
			{
		...
		CAI_BaseNPC	*pent = (CAI_BaseNPC*)CreateEntityByName( "npc_antlion" );// I'm considering adding antlion workers to the list for the episodes but i'm not sure yet.
		if ( !pent )
			break;

...
/* 		if( GetOwner()->IsPlayer() && !IsMegaPhysCannon() )
		{
			...
			if( pEntity->IsNPC() && pEntity->Classify() != CLASS_HEADCRAB && pEntity->Classify() != CLASS_ZOMBIE && pEntity->Classify() != CLASS_COMBINE && !FClassnameIs(pEntity, "npc_antlion") )
			{
				DryFire();
				return;
			}
...
bool CWeaponPhysCannon::AttachObject( CBaseEntity *pObject, const Vector &vPosition )
{
	...
	if( pOwner )
	{
		...
		bool m_bFearSprint = hl2_fear_style_movement.GetBool();
...
void CWeaponPhysCannon::DetachObject( bool playSound, bool wasLaunched )
{
	...
	if( pOwner != NULL )
	{
		...
		{
			pOwner->SetMaxSpeed( hl2_walkspeed.GetFloat() );
		}

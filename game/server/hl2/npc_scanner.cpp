...
#define SHIELD_SCANNER_FOLLOW_DIST      256

#define SHIELD_SCANNER_PHOTO_NEAR_DIST			128
...
#define SCANNER_SCOUT_MAX_SPEED			250

ConVar	sk_scanner_health( "sk_scanner_health","0");
ConVar	sk_shield_scanner_health( "sk_shield_scanner_health","0");
...
void CNPC_CScanner::Spawn(void)
{
	...
	if( m_bIsClawScanner )
	{
		...
		m_iHealth = sk_shield_scanner_health.GetFloat();
...
void CNPC_CScanner::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( !m_bIsClawScanner && GetEnemy() != NULL && (info.GetDamageType() & DMG_DISSOLVE) == false )
	{
		...
		if ( ( vecDelta.z > 0 ) && ( vecDelta.Length() > 0 ) ) // 120 320
		{	
...
void CNPC_CScanner::GatherConditions( void )
{
	...
	ClearCondition( COND_CSCANNER_CAN_PHOTOGRAPH || !m_bIsClawScanner );
	...
	if ( m_nFlyMode == SCANNER_FLY_PHOTO )
	{
		...
		if ( gpGlobals->curtime > m_fNextPhotographTime && HaveInspectTarget() && GetCurrentVelocity().LengthSqr() < (64*64) )
		{
			...
			if ( fInspectDist > SCANNER_PHOTO_NEAR_DIST && fInspectDist < SCANNER_PHOTO_FAR_DIST && !m_bIsClawScanner )//bookmark
			{
			...
			else if ( fInspectDist > SHIELD_SCANNER_PHOTO_NEAR_DIST && fInspectDist < SHIELD_SCANNER_PHOTO_FAR_DIST && m_bIsClawScanner )
			{
								// Make sure we're looking at the target
				if ( UTIL_AngleDiff( GetAbsAngles().y, VecToYaw( InspectTargetPosition() - GetAbsOrigin() ) ) < 4.0f )
				{
...
void CNPC_CScanner::AttackFlashBlind(void)
{
	...
	m_fNextSpotlightTime = gpGlobals->curtime + 0.5f;
}
...
void CNPC_CScanner::StartTask( const Task_t *pTask )
{
	...
	case TASK_CSCANNER_ATTACK_PRE_FLASH:
	{
		...
		if (m_pEyeFlash)
		{
			...
			if ( !m_bIsClawScanner )
				SetWait( SCANNER_FLASH_TIME ); 
			else
				SetWait( SHIELD_SCANNER_FLASH_TIME );
		}
		else
		{
			TaskFail("No Flash");
...
float CNPC_CScanner::GetGoalDistance( void )
{
	...
	if ( !m_bIsClawScanner )
		...
		return ( SHIELD_SCANNER_PHOTO_NEAR_DIST + ( ( SHIELD_SCANNER_PHOTO_FAR_DIST - SHIELD_SCANNER_PHOTO_NEAR_DIST ) / 2 ) );
		break;
	...
	if ( !m_bIsClawScanner )
		...
		return ( SHIELD_SCANNER_FOLLOW_DIST );
		break;
	}

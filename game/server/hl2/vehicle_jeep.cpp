...
void CPropJeep::FireChargedCannon( void )
{
	...
	//FIXME: Use ConVars!
	float flDamage = sk_jeep_gauss_charged_damage.GetFloat() + ( ( 100 - sk_jeep_gauss_charged_damage.GetFloat() ) * flChargeAmount );//bookmark1
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPropJeep::TempDisableCar( void )
{
	if ( m_flDisableTime > gpGlobals->curtime )
	{
		DevMsg("Engine Locked");
...
void CPropJeep::DriveVehicle( float flFrameTime, CUserCmd *ucmd, int iButtonsDown, int iButtonsReleased )
{
	...
	if ( m_bHasGun )
	{
		...
		if ( IsOverturned() == false && !m_bUnableToFire )
		{

...
void CNPC_Alyx::Event_KilledOther( CBaseEntity *pVictim, const CTakeDamageInfo &info )
{
	...
	if( !HasShotgun() )
	{
		...
		if( pMemory )
		{
			...
			pMemory->timeFirstSeen = gpGlobals->curtime - 10.0f;//bookmark
		}
	}
}
...
void CNPC_Alyx::DoCustomSpeechAI( void )
{
	...
	if ( HasCondition(COND_NEW_ENEMY) && GetEnemy() )
	{
		if ( GetEnemy()->Classify() == CLASS_HEADCRAB )
		{
			...
			if ( ( GetEnemy()->GetEnemy() == this ) && pHC->IsJumping() && gpGlobals->curtime - GetEnemies()->FirstTimeSeen(GetEnemy()) < 0.5 )//bookmark
			{
...
{
	/*
	// Alyx can see enemies that are right next to her //bookmark
	...
	case SCHED_HIDE_AND_RELOAD:
		{
			...
			// If I don't have a ranged attacker as an enemy, don't try to hide. bookmark
			AIEnemiesIter_t iter;
			for ( AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst(&iter); pEMemory != NULL; pEMemory = GetEnemies()->GetNext(&iter) )
			{
				...
				// Look for enemies with ranged capabilities. bookmark

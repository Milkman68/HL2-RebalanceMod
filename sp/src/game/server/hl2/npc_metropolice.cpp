...
void CNPC_MetroPolice::PrescheduleThink( void )
{
	...
	if( gpGlobals->curtime >= m_flStopMoveShootTime && !ShouldMoveAndShoot() )
	{
	...
	if( IsOnFire() )
	{
		...
		m_MoveAndShootOverlay.SuspendMoveAndShoot( 5000.0f );
	}
	else
	{
...
{
	if( IsOnFire() && IsCurSchedule( SCHED_METROPOLICE_BURNING_RUN ) )
	{
...
int CNPC_MetroPolice::SelectCombatSchedule()
{
	...
	if( !m_fWeaponDrawn && !Weapon_OwnsThisType( "weapon_smg1" ) )
	{
		return SCHED_METROPOLICE_DRAW_PISTOL;
	}
	...
	if ( GetEnemy() && !HasCondition( COND_SEE_ENEMY ) )
	{
		...
		// if ( OccupyStrategySlot( SQUAD_SLOT_POLICE_COVERING_FIRE1 ) && !HasBaton() )
		// {
...
// TraceAttack
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
...
/* 	if ( GetEnemy() && GetActiveWeapon() )
	...
	if( HasCondition( COND_NO_PRIMARY_AMMO ) && !HasBaton() )
	{
		AnnounceOutOfAmmo( );
		return SCHED_METROPOLICE_HIDE_AND_RELOAD;
	}
	...
	if ( m_NPCState == NPC_STATE_COMBAT && m_flChasePlayerTime > gpGlobals->curtime && HasBaton() )
...
int CNPC_MetroPolice::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	...
	case SCHED_METROPOLICE_TAKE_COVER_FROM_ENEMY:
	case SCHED_METROPOLICE_WAIT_IN_COVER:
...
/* 			case 2:	
	...
	case SCHED_RANGE_ATTACK1:
		...
		if ( HasCondition( COND_NO_PRIMARY_AMMO ) || HasCondition( COND_LOW_PRIMARY_AMMO ) )
		{
			...
			VacateStrategySlot();
			return TranslateSchedule( SCHED_METROPOLICE_HIDE_AND_RELOAD );
		}
...
bool CNPC_MetroPolice::CanDeployManhack( void )
{
	...
	int m_iManhacksInSquad = 0;

	while ( pSquadmate )
	{
		CNPC_Manhack *pManhack = dynamic_cast<CNPC_Manhack*>(pSquadmate);
...
void CNPC_MetroPolice::GatherConditions( void )
{
	...
	ClearCondition( COND_ATTACK_SLOT_AVAILABLE );
...
/* 	if ( IsCurSchedule( SCHED_METROPOLICE_CHASE_ENEMY ) )
	{
		if( metropolice_chase_use_follow.GetBool() && GetEnemy() )
		{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CNPC_MetroPolice::CanBecomeElite( void )
{
	char szMapName[256];
	Q_strncpy(szMapName, STRING(gpGlobals->mapname), sizeof(szMapName) );
	Q_strlower(szMapName);
		
	// List of Trainstation maps where Elites can appear.
	...
	// List of canal maps where Elites can appear.
	bool m_bInCanals = ( 
	...
	// If our map's name starts with d3_c17, then we're in a city map.
	bool m_bInC17 = ( !Q_strnicmp( szMapName, "d3_c17", 6 ) );
	
	// Makes elites very rare in the beginning of the game.
	if ( m_bInTrainstation && ( random->RandomInt(0,100) > 90) )	
	{
		return true;
	}
	
	if ( ( m_bInCanals || m_bInC17 ) )
	{
...
(
	...
	"		TASK_GET_CHASE_PATH_TO_ENEMY	10"

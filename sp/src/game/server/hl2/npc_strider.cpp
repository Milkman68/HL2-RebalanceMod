...
#define STRIDER_TIME_STOP_FACING_ENEMY 6.0 
...
/* 		if( m_hPlayersMissile )
	...
	if( !m_bCrouchLocked && !m_bDontCrouch )
	{
		...
		{
			if ( strider_distributed_fire.GetBool() && !IsUsingAggressiveBehavior() )//bookmark1
			{
				m_iVisibleEnemies = 0;
...
/* 			if( bPlayer )
			...
			// Interrupt our gun during the flinch bookmark1
...
float CNPC_Strider::MaxYawSpeed()
{
	...
	default:
		return 15;		// should be zero, but this makes it easy to see when he's turning with default AI code
		break;
	}
}
...
void CNPC_Strider::ShootMinigun( const Vector *pTarget, float aimError, const Vector &vecSpread )
{
	if ( pTarget )
	{
		...
		if( m_bMinigunUseDirectFire )
		{
			...
			FireBullets( 1, muzzlePos, vecShootDir, vecSpread, 8192, m_miniGunDirectAmmo, 1 );//bookmark1. 4th number might be speed of tracer.
		}
		else
		{
			// exactly on target w/tracer

...
ConVar	sk_manhack_charge_Duration( "sk_manhack_charge_Duration","1");
ConVar	sk_manhack_charge_interval( "sk_manhack_charge_interval","2");

ConVar	sk_manhack_v2( "sk_manhack_v2","1");

ConVar	sk_manhack_die_after_slice( "sk_manhack_die_after_slice","0");
...
BEGIN_DATADESC( CNPC_Manhack )
	...
	DEFINE_FIELD( m_bIsEyeStunned,			FIELD_BOOLEAN ),
...
IMPLEMENT_SERVERCLASS_ST(CNPC_Manhack, DT_NPC_Manhack)
	...
	SendPropBool( SENDINFO( m_bIsEyeStunned ) ),
	SendPropBool( SENDINFO( m_bIsCharging ) ),
	SendPropBool( SENDINFO( m_bIsDead ) )
END_SEND_TABLE()
...
void CNPC_Manhack::HandleAnimEvent( animevent_t *pEvent )
{
	...
	case MANHACK_AE_DONE_UNPACKING:
		m_flNextBurstTime = sk_manhack_charge_interval.GetFloat();
		break;
...
void CNPC_Manhack::Slice( CBaseEntity *pHitEntity, float flInterval, trace_t &tr )
{
	...
	if ( sk_manhack_die_after_slice.GetBool() )//bookmark
	{
		if ( pHitEntity->IsPlayer() || pHitEntity->IsNPC() )
		{
			CTakeDamageInfo info1;

			info1.SetAttacker( this );
			info1.SetInflictor( this );
			info1.SetDamage( m_iHealth );
...
{
	if( m_flWaterSuspendTime > gpGlobals->curtime )//bookmark
	{
		// Slower in water!
...
void CNPC_Manhack::StartBurst( const Vector &vecDirection )
{
	...
	m_flNextBurstTime = gpGlobals->curtime + sk_manhack_charge_interval.GetFloat(); //2
	m_flBurstDuration = gpGlobals->curtime + sk_manhack_charge_Duration.GetFloat(); //1

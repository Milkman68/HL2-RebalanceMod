...
ConVar sk_zombie_poison_dmg_slash( "sk_zombie_poison_dmg_slash","0");
...
void CNPC_PoisonZombie::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_LEFT )
	{
		...
		forward = forward * 200;
		ClawAttack( GetClawAttackRange(), sk_zombie_poison_dmg_slash.GetFloat(), qaPunch, forward, ZOMBIE_BLOOD_BITE );
		return;
	}

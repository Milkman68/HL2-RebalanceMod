...
ConVar	sk_antlion_swipe_damage_combine( "sk_antlion_swipe_damage_combine", "0" );
...
void CNPC_Antlion::MeleeAttack( float distance, float damage, QAngle &viewPunch, Vector &shove )
{
	...
	if ( pHurt )
	{
		...
		{
			CTakeDamageInfo	dmgInfo( this, this, sk_antlion_swipe_damage_combine.GetFloat() - sk_antlion_swipe_damage.GetFloat(), DMG_SLASH );
...
/*bool CNPC_Antlion::ChargeConditions( void )//bookmark
{
	float flDistToEnemy = ( GetEnemy()->GetAbsOrigin() - GetAbsOrigin() ).Length();
	
	if ( flDistToEnemy < 300 )
		return false;
	if ( 
}*/
...
int CNPC_Antlion::SelectSchedule( void )
{
	...
	if ( HasCondition( COND_HEAR_BUGBAIT ) && ( IsAllied() ) ) //bookmark
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_Antlion::IsAllied(void)//bookmark
{
  CBaseEntity *pGuard = gEntList.FindEntityByClassnameWithin( NULL, "npc_antlionguard", GetLocalOrigin(), 2000);
  
  // Antlions are always hostile when a guard is present
  if (pGuard)
    return false;
...
// Purpose: 
//-----------------------------------------------------------------------------
/*bool CNPC_Antlion::SMG1Damage( const CTakeDamageInfo &info )
{

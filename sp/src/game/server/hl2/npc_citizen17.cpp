...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Citizen::PredictPlayerPush()//bookmark2
{
	if ( !AI_IsSinglePlayer() )
		return;
...
bool CNPC_Citizen::ShouldLookForBetterWeapon()
{
	if ( BaseClass::ShouldLookForBetterWeapon() )
	{
		...
		if( pWeapon )
		{
			...
			if( FClassnameIs( pWeapon, "weapon_ar2" ) )//bookmark4
			{
...
{
	if ( m_bCasualNotCommandable )//bookmark3
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Citizen::TogglePlayerSquadState()//bookmark1
{
	if ( !AI_IsSinglePlayer() )
		return;
	...
	{
		if ( !HasCondition ( SF_CITIZEN_NOT_COMMANDABLE ))
		{
			AddSpawnFlags( SF_CITIZEN_NOT_COMMANDABLE );
			m_bCasualNotCommandable = true;
		}
		SpeakCommandResponse( TLK_STOPFOLLOW );
		RemoveFromPlayerSquad();
		ClearFollowTarget();

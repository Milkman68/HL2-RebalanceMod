...
bool CSingleplayRules::Damage_ShouldNotBleed( int iDmgType )
{
	...
	{
		//Must have ammo
		if ( ( pWeapon->HasAnyAmmo() == false ) && ( pPlayer->GetAmmoCount( pWeapon->GetPrimaryAmmoType(CBaseCombatWeapon::INDEX_CARRY) ) <= 0 ) )
			return false;
	...
	CBaseCombatWeapon *CSingleplayRules::GetNextBestWeapon( CBaseCombatCharacter *pPlayer, CBaseCombatWeapon *pCurrentWeapon )
	{
		...
		for ( int i = 0; i < pPlayer->WeaponCount(); i++ )
		{
			...
			if ( pWeapon->UsesClipsForAmmo1() && pWeapon->Clip1() <= 0 && !pPlayer->GetAmmoCount( pWeapon->GetPrimaryAmmoType(CBaseCombatWeapon::INDEX_CARRY) ) )
				continue;
